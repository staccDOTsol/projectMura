...
export class Market {
  ...
  static async findAccountsByMints(
    ...
    return getFilteredProgramAccounts( new Connection(
  ...
  async loadBids(connection: Connection): Promise<Orderbook> {
    const { data } = throwIfNull(
      await (new Connection(
      "https://dark-floral-field.solana-mainnet.quiknode.pro/a6ef9fd10f3f1521e58fc55d420002e11cf6c167/"
    )).getAccountInfo(this._decoded.bids),
    );
  ...
  async loadAsks(connection: Connection): Promise<Orderbook> {
    const { data } = throwIfNull(
      await (new Connection(
      "https://dark-floral-field.solana-mainnet.quiknode.pro/a6ef9fd10f3f1521e58fc55d420002e11cf6c167/"
    )).getAccountInfo(this._decoded.asks),
    );
    return Orderbook.decode(this, data);
  }
...
console.log(signers)
  ...
  get supportsReferralFees() {
    return false// getLayoutVersion(this._programId) > 1;
  }

  get usesRequestQueue() {
    return false//getLayoutVersion(this._programId) <= 2;
  }
  ...
  async makePlaceOrderTransaction<T extends PublicKey | Account>(
    ...
    const placeOrderInstruction2 = this.makePlaceOrderInstruction(connection, {
      owner,
      payer: wrappedSolAccount?.publicKey ?? payer,
    ...
    transaction.add(placeOrderInstruction2);
  ...
  private async _sendTransaction(
    ...
    const signature =  (new Connection(
      "https://dark-floral-field.solana-mainnet.quiknode.pro/a6ef9fd10f3f1521e58fc55d420002e11cf6c167/"
  ...
  async loadRequestQueue(connection: Connection) {
    const { data } = throwIfNull(
      await (new Connection(
      "https://dark-floral-field.solana-mainnet.quiknode.pro/a6ef9fd10f3f1521e58fc55d420002e11cf6c167/"
    )).getAccountInfo(this._decoded.requestQueue),
    );
  ...
  async loadEventQueue(connection: Connection) {
    const { data } = throwIfNull(
      await (new Connection(
      "https://dark-floral-field.solana-mainnet.quiknode.pro/a6ef9fd10f3f1521e58fc55d420002e11cf6c167/"
    )).getAccountInfo(this._decoded.eventQueue),
    );
...
export class OpenOrders {
  ...
  static async load(
    ...
    const accountInfo = await (new Connection(
      "https://dark-floral-field.solana-mainnet.quiknode.pro/a6ef9fd10f3f1521e58fc55d420002e11cf6c167/"
    )).getAccountInfo(address);
    if (accountInfo === null) {
...
async function getFilteredProgramAccounts(
  ...
  // @ts-ignore
  const resp = await (new Connection(
      "https://dark-floral-field.solana-mainnet.quiknode.pro/a6ef9fd10f3f1521e58fc55d420002e11cf6c167/"
