...
export async function loadCandyProgram2(walletKeyPair: Keypair, env: string,
  customRpcUrl?: string) {
   let CANDY_MACHINE_PROGRAM_ID2 = new PublicKey("4gejeUfH61BLKJ4cWL1pN5NJBWRQtMfepZgEJ79yWxvu")
  // @ts-ignore
  const solConnection = new anchor.web3.Connection(
    //@ts-ignore
    customRpcUrl || 'http://localhost:8899',
  );
    const walletWrapper = new anchor.Wallet(walletKeyPair);
  const provider = new anchor.Provider(solConnection, walletWrapper, {
    preflightCommitment: 'recent',
  });
  const idl = await anchor.Program.fetchIdl(CANDY_MACHINE_PROGRAM_ID2, provider);

  const program = new anchor.Program(idl, CANDY_MACHINE_PROGRAM_ID2, provider);
